...
public class AntColony {
    ...
    int peakCount, actualpeakCount, aaCount, antCount,  alpha, beta, gamma,
        cnstrntPkCnt, nextPeakSelect, constForInfeasible,
    ...
    double evaporationConst, pheromoneInitialVal, infeasibleScore, localPhrmnUpdtValue,unavailableValue,
    ...
    String paramsFile = "Parameters.txt", binary_HN_HN, binary_HN_HA,binary_HA_HN, combinedScoreFile, noelist_HN_HN, noelist_HN_HA,noelist_HA_HN, resultFile, matlabFile;
    ArrayList<Integer> noe_HN_HA[],noe_HA_HN[],noe_HN_HN[], candidateList[], candidateListAA[];
    ArrayList<Float> UB_HN_HN[],UB_HN_HA[],UB_HA_HN[];
    ...
    public static void main(String[] args) {
        try {
            ...
            String inputListFileName = "input.txt";
            BufferedReader input = new BufferedReader(new FileReader(inputListFileName));
			String line;
			int runNo=1;
			while((line = input.readLine()) != null)
			{
				String[] values = line.split("\\s+");
				//The parameters have to be given in the general form
                app.paramsFile = values[0];
                runCount = Integer.parseInt(values[1]);
                alpha1 = Integer.parseInt(values[2]);
                alpha2 = Integer.parseInt(values[3]);
                beta1 = Integer.parseInt(values[4]);
                beta2 = Integer.parseInt(values[5]);
			}
    ...
    private void mainProgram() {
        try {
            ...
            if (readBinaryDistance() < 0) {
                System.out.println("Error in reading file " + binary_HN_HA);
                System.out.println("Error in reading file " + binary_HA_HN);
                System.out.println("Error in reading file " + binary_HN_HN);
            }
            if (readNoeList() < 0) {
                System.out.println("Error in reading file " + noelist_HN_HA);
                System.out.println("Error in reading file " + noelist_HA_HN);
                System.out.println("Error in reading file " + noelist_HN_HN);
            }
    ...
    private void readParameters() {
        try {
            ...
            while ((s = br.readLine()) != null) {
               ...
               if (str.compareTo("actualpeak") == 0) {
                   actualpeakCount = Integer.parseInt(st.nextToken());
                   System.out.println(str + " = " + actualpeakCount);
               }
               if (str.compareTo("aminoacid") == 0) {
                   aaCount = Integer.parseInt(st.nextToken());
                   System.out.println(str + " = " + aaCount);
                   aaCount = aaCount+1;  //seyma
                   System.out.println(" aaCount is increased to " + aaCount + " for dummy aa ");   //seyma
               }
               ...
               if (str.compareTo("unavailableValue") == 0) {
                   unavailableValue = Double.parseDouble(st.nextToken());
               ...
               if (str.compareTo("binary_HN_HN") == 0) {
                   binary_HN_HN = st.nextToken();
                   System.out.println(str + " = " + binary_HN_HN);
               }
               if (str.compareTo("binary_HN_HA") == 0) {
                   binary_HN_HA = st.nextToken();
                   System.out.println(str + " = " + binary_HN_HA);
               }
               if (str.compareTo("binary_HA_HN") == 0) {
                   binary_HA_HN = st.nextToken();
                   System.out.println(str + " = " + binary_HA_HN);
               }
               if (str.compareTo("combinedScoreFile") == 0) {
                   combinedScoreFile = st.nextToken();
                   System.out.println(str + " = " + combinedScoreFile);
               }
               if (str.compareTo("noelist_HN_HN") == 0) {
            	   noelist_HN_HN = st.nextToken();
                   System.out.println(str + " = " + noelist_HN_HN);
               }
               if (str.compareTo("noelist_HN_HA") == 0) {
            	   noelist_HN_HA = st.nextToken();
                   System.out.println(str + " = " + noelist_HN_HA);
               }
               if (str.compareTo("noelist_HA_HN") == 0) {
            	   noelist_HA_HN = st.nextToken();
                   System.out.println(str + " = " + noelist_HA_HN);
               }
    ...
    private int readCombinedScore() {
        try {
            ...
            while ((s = br.readLine()) != null) {
                st = new StringTokenizer(s);
                for (j=0; j<aaCount-1; j++)  //seyma  (aaCount'u en basta 1 artirdigim icin)
    ...
    private double getInfeasibleScore() {
        try {
            ...
            int i, j;
            for (i = 0; i < peakCount; i++)
            for (j = 0; j < aaCount-1; j++)  //seyma -1 (aaCount en basta 1 artitrilmisti)
                if ((combinedScore[i][j] != 1000000000) && (combinedScore[i][j] > res))
                    res = combinedScore[i][j];
            for (i = 0; i < peakCount; i++)  //seyma
                combinedScore[i][aaCount-1] = res;   //seyma
    ...
    private void runAnts() {
        ...
        Ant ant[] = new Ant[antCount],
            bestAnt = new Ant(peakCount, actualpeakCount, combinedScore, HN_HN,HN_HA,HA_HN, noe_HN_HN,noe_HN_HA,noe_HA_HN,
            		UB_HN_HN, UB_HN_HA, UB_HA_HN, infeasibleScore, alpha, gamma, //beta,
        ...
        for (i = 0; i < antCount; i++) {
            ant[i] = new Ant(i, actualpeakCount, combinedScore, HN_HN, HN_HA,HA_HN, noe_HN_HN, noe_HN_HA,noe_HA_HN, 
            		UB_HN_HN, UB_HN_HA, UB_HA_HN,infeasibleScore, alpha, gamma, //beta,
                    pheromone, heuristicBetaPower, unavailableSelect, unavailableValue, infeasibleSelect,
                    localPhrmnUpdtValue, cnstrntPkCnt, nextPeakSelect, localSearchStop, totalScoreCalcCoef,
                    candidateListValue * aaCount / 100, candidateList, candidateListAA, scoreConstraints);
...
//                    System.out.print(iterationBest[j] + ": " + ant[iterationBest[j]].getTotalScore());
                            switch (localSearchVersion) {                                       // make elitist local search
                                ...
                                case 3:
                                	...
                                	System.out.println("Three opt çiktiiiiiiiiiiiiiiiii");
                                    break;
                            }
...
//                    fr.write("<" + j + "-" + goodAssignmentList.get(i).assignment[j].aminoAcid + ", " + goodAssignmentList.get(i).assignment[j].feasibility + "> ");
    ...
    try {
            fr = new FileReader(binary_HN_HN);
            br = new BufferedReader(fr);
            String s;
            StringTokenizer st;
            this.HN_HN = new float[aaCount][aaCount]; 
            int i = 0, j;

            while ((s = br.readLine()) != null) {
                st = new StringTokenizer(s);
                for (j=0; j<aaCount-1; j++)  //seyma  -1 (aaCount'u enbasta bir artırmistim)
                    this.HN_HN[i][j] = Float.parseFloat(st.nextToken());
                this.HN_HN[i][j] = 1;  //seyma  
                i++;
           }    
            for (j=0; j<aaCount; j++)   //seyma
                    this.HN_HN[i][j] = 1;   //seyma
            System.out.println(binary_HN_HN + " is succesfully migrated.");
            br.close();
            fr.close();
            one=1;
        } catch (Exception exc) { exc.printStackTrace(System.out); }        
    ...
    try {
        fr = new FileReader(binary_HN_HA);
        br = new BufferedReader(fr);
        String s;
        StringTokenizer st;
        this.HN_HA = new float[aaCount][aaCount]; 
        int i = 0, j;

        while ((s = br.readLine()) != null) {
            st = new StringTokenizer(s);
            for (j=0; j<aaCount-1; j++)   //seyma  -1 (aaCount'u enbasta bir artırmistim)
                this.HN_HA[i][j] = Float.parseFloat(st.nextToken());
            this.HN_HA[i][j] = 1;  //seyma
            i++;
       }    
        for (j=0; j<aaCount; j++)   //seyma
                    this.HN_HA[i][j] = 1;   //seyma
        System.out.println(binary_HN_HA + " is succesfully migrated.");
        br.close();
        fr.close();
        two=1;
    } catch (Exception exc) { exc.printStackTrace(System.out); }
    ...
    try {
        fr = new FileReader(binary_HA_HN);
        br = new BufferedReader(fr);
        String s;
        StringTokenizer st;
        this.HA_HN = new float[aaCount][aaCount];
        int i = 0, j;

        while ((s = br.readLine()) != null) {
            st = new StringTokenizer(s);
            for (j=0; j<aaCount-1; j++)  //seyma  -1 (aaCount'u enbasta bir artırmistim)
                this.HA_HN[i][j] = Float.parseFloat(st.nextToken());
            this.HA_HN[i][j] = 1;   //seyma
            i++;
       }
        for (j=0; j<aaCount; j++)   //seyma
                    this.HA_HN[i][j] = 1;   //seyma
        System.out.println(binary_HA_HN + " is succesfully migrated.");
        br.close();
        fr.close();
        three=1;
    } catch (Exception exc) { exc.printStackTrace(System.out); }
    ...
    private int readNoeList() {     // reading noe values from file to noe arrays
        ...
        try {
            fr = new FileReader(noelist_HN_HN);
            br = new BufferedReader(fr);
            String s;
            StringTokenizer st;
            int peak, row = 0, i, count, noePeak;
            this.noe_HN_HN = new ArrayList[peakCount];
            this.UB_HN_HN = new ArrayList[peakCount];
            for (i=0; i<peakCount; i++) {
            	this.noe_HN_HN[i] = new ArrayList<Integer>();
            	this.UB_HN_HN[i] = new ArrayList<Float>();}


            while ((s = br.readLine()) != null) {
                st = new StringTokenizer(s);
                count = st.countTokens();                  // number of noe peaks
                peak = Integer.parseInt(st.nextToken()) - 1;                // number of peak
                for (i = 0; i < count-1; i++) {
                	String [] cell =st.nextToken().split(":");
                    noePeak = Integer.parseInt(cell[0])-1;
                    if (!this.noe_HN_HN[peak].contains(noePeak) && peak!=noePeak ) {
                    	this.noe_HN_HN[peak].add(noePeak);
                    	this.UB_HN_HN[peak].add(Float.parseFloat(cell[1]));
                    	if(!this.noe_HN_HN[noePeak].contains(peak)){
                    	this.noe_HN_HN[noePeak].add(peak);
                    	this.UB_HN_HN[noePeak].add(Float.parseFloat(cell[1]));}
                    }
                }
            }
            System.out.println(noelist_HN_HN + " is succesfully migrated.");
            br.close();
            fr.close();
            one=1;
        } catch (Exception exc) { exc.printStackTrace(System.out);  }        
        ...
        try {
            fr = new FileReader(noelist_HN_HA);
            br = new BufferedReader(fr);
            String s;
            StringTokenizer st;
            int peak, row = 0, i, count, noePeak;
            this.noe_HN_HA = new ArrayList[peakCount];
            this.UB_HN_HA = new ArrayList[peakCount];
            for (i=0; i<peakCount; i++) {this.noe_HN_HA[i] = new ArrayList<Integer>();
            this.UB_HN_HA[i] = new ArrayList<Float>();}
            this.noe_HA_HN = new ArrayList[peakCount];
            this.UB_HA_HN = new ArrayList[peakCount];
            for (i=0; i<peakCount; i++) {this.noe_HA_HN[i] = new ArrayList<Integer>();
            this.UB_HA_HN[i] = new ArrayList<Float>();}
            ...
            while ((s = br.readLine()) != null) {
                ...
                for (i = 0; i < count-1; i++) {
                	String cell[]=st.nextToken().split(":");
                    noePeak = Integer.parseInt(cell[0])-1;
                    if (!this.noe_HN_HA[peak].contains(noePeak) && peak!=noePeak ) {
                    	this.noe_HN_HA[peak].add(noePeak);
                    	this.UB_HN_HA[peak].add(Float.parseFloat(cell[1]));
                    	if(!this.noe_HA_HN[noePeak].contains(peak)){
                    	this.noe_HA_HN[noePeak].add(peak);
                    	this.UB_HA_HN[noePeak].add(Float.parseFloat(cell[1]));}
                    }
                }
            }
            System.out.println(noelist_HN_HA + " is succesfully migrated.");
            br.close();
            fr.close();
            two=1;
        } catch (Exception exc) { exc.printStackTrace(System.out);  }
        ...
        try {
            fr = new FileReader(noelist_HA_HN);
            br = new BufferedReader(fr);
            String s;
            StringTokenizer st;
            int peak, row = 0, i, count, noePeak;
            ...
            while ((s = br.readLine()) != null) {
                ...
                for (i = 0; i < count-1; i++) {
                	String cell[]=st.nextToken().split(":");
                    noePeak = Integer.parseInt(cell[0])-1;
                    if (!this.noe_HA_HN[peak].contains(noePeak) && peak!=noePeak) {
                    	this.noe_HA_HN[peak].add(noePeak);
                    	this.UB_HA_HN[peak].add(Float.parseFloat(cell[1]));
                    	if(!this.noe_HN_HA[noePeak].contains(peak)){
                    	this.noe_HN_HA[noePeak].add(peak);
                    	this.UB_HN_HA[noePeak].add(Float.parseFloat(cell[1]));}
                    }
                }
            }
            System.out.println(noelist_HA_HN + " is succesfully migrated.");
            br.close();
            fr.close();
            three= 1;
        } catch (Exception exc) { exc.printStackTrace(System.out);  }
        ...
        for(int i=0; i<this.noe_HN_HA.length; i++){
        	System.out.print(i+"-->");
        	for(int j=0; j<this.noe_HN_HA[i].size(); j++){
        		System.out.print(this.noe_HN_HA[i].get(j)+"\t");
        	}System.out.println();
        }
        
        System.out.println("UB_HN_HA");
        for(int i=0; i<this.UB_HN_HA.length; i++){
        	System.out.print(i+"-->");
        	for(int j=0; j<this.UB_HN_HA[i].size(); j++){
        		System.out.print(this.UB_HN_HA[i].get(j)+"\t");
        	}System.out.println();
        }
        
        System.out.println("HA_HN");
        for(int i=0; i<this.noe_HA_HN.length; i++){
        	System.out.print(i+"-->");
        	for(int j=0; j<this.noe_HA_HN[i].size(); j++){
        		System.out.print(this.noe_HA_HN[i].get(j)+"\t");
        	}System.out.println();
        }
        
        System.out.println("UB_HA_HN");
        for(int i=0; i<this.UB_HA_HN.length; i++){
        	System.out.print(i+"-->");
        	for(int j=0; j<this.UB_HA_HN[i].size(); j++){
        		System.out.print(this.UB_HA_HN[i].get(j)+"\t");
        	}System.out.println();
        }
        
        System.out.println("HN_HN");
        for(int i=0; i<this.noe_HN_HN.length; i++){
        	System.out.print(i+"-->");
        	for(int j=0; j<this.noe_HN_HN[i].size(); j++){
        		System.out.print(this.noe_HN_HN[i].get(j)+"\t");
        	}System.out.println();
        }
        
        System.out.println("UB_HN_HN");
        for(int i=0; i<this.UB_HN_HN.length; i++){
        	System.out.print(i+"-->");
        	for(int j=0; j<this.UB_HN_HN[i].size(); j++){
        		System.out.print(this.UB_HN_HN[i].get(j)+"\t");
        	}System.out.println();
        }
...
//        System.out.println();
    ...
    private double calculateDiagonalScore() {
        ...
        for (int i=actualpeakCount; i<peakCount; i++) //seyma
            res += combinedScore[peakCount-1][aaCount-1];   //seyma    fazla olan peaklerin atanması
        return res;
    }
